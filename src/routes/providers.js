...
const { debugSomeCommonImages } = require('../controllers/providers');
...
router.post('/scrape', private, async (req, res, next) => {
  ...
  res.status(202).json({ message: "Started scrape process", data: {id: req.requestId}});
...
router.post('/debugSomeCommonImages', private, async (req, res, next) => {
  // set status to 202: the request has been accepted for processing, but the processing has not been completed. See https://tools.ietf.org/html/rfc7231#section-6.3.3.
  res.status(202).json({ message: "Started debugging some common images process", data: { id: req.requestId } });

  await debugSomeCommonImages(req, res, next);
  logger.info('finished debugging some common images');
});
